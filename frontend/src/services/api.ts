...
// Attempt to refresh token on 401 responses and retry the original request once
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const isAuthEndpoint = url.endsWith('/token/') || url.endsWith('/token/refresh/');

    // Avoid infinite loops using a custom flag
