...
export const Notifications: React.FC = () => {
  ...
  // Pobierz ustawienia powiadomień z profilu
  useEffect(() => {
  ...
  useEffect(() => {
    loadNotifications();
    // Odśwież powiadomienia co 30 sekund
    const interval = setInterval(loadNotifications, 30000);
    return () => clearInterval(interval);
  }, []);

  // Użyj hooka do powiadomień systemowych
        ...
        <DialogContent>
          {loading ? (
            ...
            <Box textAlign="center" py={4}>
              ...
              <Typography color="textSecondary">Brak powiadomień</Typography>
            </Box>
          ) : (
            <List>
                  ...
                  >
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'flex-start', width: '100%', gap: 2 }}>
                        <DirectionsCar
                          sx={{
                            color: notification.read ? '#9e9e9e' : '#c62828',
                            mt: 0.5,
                          }}
                        />
                          ...
                          {notification.trip_info && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1, color: '#757575' }}>
                              <Typography variant="caption">
                                {notification.trip_info.start_location}
                              </Typography>
                              <ArrowForward fontSize="small" />
                              <Typography variant="caption">
                                {notification.trip_info.end_location}
                              </Typography>
                              <Typography variant="caption" sx={{ ml: 1 }}>
                                • {notification.trip_info.price_per_seat} zł
                              </Typography>
                            </Box>
                          )}
