      ...
      <Box sx={{ bgcolor: '#e0e0e0', pb: 4 }}>
        <AppBar position="static" color="transparent" elevation={0}>
          <Toolbar sx={{ justifyContent: 'space-between' }}>
            <Typography variant="h6" component="div" sx={{ fontWeight: 'bold', color: '#000', ml: 4, cursor: 'pointer' }} onClick={() => navigate('/')}>Sheero</Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Button color="inherit" sx={{ textTransform: 'none' }} onClick={() => navigate('/search')}>Wyszukaj</Button>
              <IconButton onClick={() => navigate('/profile')}><Avatar sx={{ width: 40, height: 40, border: '2px solid white' }} /></IconButton>
              <IconButton onClick={() => { authService.logout(); navigate('/login'); }} title="Wyloguj"><Logout /></IconButton>
            </Box>
          </Toolbar>
        </AppBar>
        <Container maxWidth="lg">
          <Paper component="form" onSubmit={formik.handleSubmit} elevation={0} sx={{ p: '8px', borderRadius: '50px', bgcolor: '#f5f5f5', display: 'flex', alignItems: 'center', gap: 1, boxShadow: '0px 2px 10px rgba(0,0,0,0.05)', mx: 4, flexWrap: 'wrap' }}>
            <Paper sx={{ px: 2, py: 0.5, borderRadius: '30px', flex: 1.5, display: 'flex', alignItems: 'center', boxShadow: '0px 1px 3px rgba(0,0,0,0.1)' }}>
              <TextField fullWidth placeholder="Miejsce wyjazdu" variant="standard" name="start_location" value={formik.values.start_location} onChange={formik.handleChange} InputProps={{ disableUnderline: true, endAdornment: formik.values.start_location && <IconButton size="small" onClick={() => formik.setFieldValue('start_location', '')}><CloseIcon fontSize="small" /></IconButton> }} />
            </Paper>
            <ArrowForward color="action" />
            <Paper sx={{ px: 2, py: 0.5, borderRadius: '30px', flex: 1.5, display: 'flex', alignItems: 'center', boxShadow: '0px 1px 3px rgba(0,0,0,0.1)' }}>
              <TextField fullWidth placeholder="Miejsce docelowe" variant="standard" name="end_location" value={formik.values.end_location} onChange={formik.handleChange} InputProps={{ disableUnderline: true, endAdornment: formik.values.end_location && <IconButton size="small" onClick={() => formik.setFieldValue('end_location', '')}><CloseIcon fontSize="small" /></IconButton> }} />
            </Paper>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, px: 3, borderLeft: '1px solid #ccc', borderRight: '1px solid #ccc', height: '30px' }}>
              <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 1, color: '#424242', cursor: 'pointer' }}>
                ...
                <Typography variant="body2" sx={{ whiteSpace: 'nowrap', pointerEvents: 'none' }}>{formik.values.date ? getDisplayDate(formik.values.date) : 'Najbliższy miesiąc'}</Typography>
                <TextField type="date" name="date" value={formik.values.date} onChange={formik.handleChange} variant="standard" InputProps={{ disableUnderline: true }} sx={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer', '& input': { cursor: 'pointer', height: '100%' } }} />
              </Box>
              <Box onClick={handlePassengerClick} sx={{ display: 'flex', alignItems: 'center', gap: 1, color: '#424242', cursor: 'pointer' }}>
                <PersonOutline />
                <Typography variant="body2" sx={{ whiteSpace: 'nowrap' }}>{passengerCount} {passengerCount === 1 ? 'pasażer' : 'pasażerów'}</Typography>
              </Box>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <IconButton onClick={handleSaveAsFavorite} disabled={!formik.values.start_location || !formik.values.end_location} sx={{ color: isCurrentRouteFavorite() ? '#FFD700' : '#757575', '&:hover': { color: '#FFD700' } }} title={isCurrentRouteFavorite() ? 'Trasa jest już w ulubionych' : 'Zapisz trasę jako ulubioną'}>
                {isCurrentRouteFavorite() ? <Star /> : <StarBorder />}
              </IconButton>
              <Button type="submit" variant="contained" sx={{ borderRadius: '30px', bgcolor: '#c62828', px: 4, py: 1, textTransform: 'none', fontSize: '1rem', minWidth: '120px', '&:hover': { bgcolor: '#b71c1c' } }}>Szukaj</Button>
            </Box>
          </Paper>
        </Container>
      </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 6 }}>
          <Box sx={{ width: { xs: '100%', md: '25%' } }}>
            <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: '20px', bgcolor: '#f5f5f5' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#000' }}>Ulubione trasy</Typography>
                <Star sx={{ color: '#FFD700', fontSize: '20px' }} />
              </Box>
              {loadingFavorites ? (
                <Box display="flex" justifyContent="center" py={2}><CircularProgress size={24} /></Box>
              ) : favoriteRoutes.length === 0 ? (
                <Typography variant="body2" color="textSecondary" sx={{ textAlign: 'center', py: 2 }}>Brak ulubionych tras. Zapisz trasę klikając gwiazdkę przy wyszukiwaniu.</Typography>
              ) : (
                <Stack spacing={1}>
                  {favoriteRoutes.map((route) => (
                    <Paper key={route.id} elevation={0} sx={{ p: 1.5, borderRadius: '15px', bgcolor: 'white', cursor: 'pointer', border: '1px solid #e0e0e0', '&:hover': { borderColor: '#c62828', bgcolor: '#ffebee' }, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Box onClick={() => handleUseFavoriteRoute(route)} sx={{ flex: 1, minWidth: 0 }}>
                        <Typography variant="body2" sx={{ fontWeight: 'bold', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{route.start_location}</Typography>
                        <Typography variant="caption" color="textSecondary" sx={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{route.end_location}</Typography>
                      </Box>
                      <IconButton size="small" onClick={(e) => { e.stopPropagation(); if (route.id) handleDeleteFavorite(route.id); }} sx={{ color: '#757575', '&:hover': { color: '#c62828' } }} title="Usuń z ulubionych"><DeleteIcon fontSize="small" /></IconButton>
                    </Paper>
                  ))}
                </Stack>
              )}
          ...
          <Box sx={{ width: { xs: '100%', md: '75%' } }}>
            <Paper elevation={0} sx={{ bgcolor: '#e0e0e0', py: 1.5, px: 3, borderRadius: '20px', mb: 4, textAlign: 'center' }}>
              <Typography variant="body1">{loading ? 'Szukanie...' : `Wyszukało ${trips.length} przejazdy`}</Typography>
            </Paper>
            <Stack spacing={3}>
              {loading ? (
                ...
                sortedTrips.map((trip) => (
                  <Paper key={trip.id} elevation={0} onClick={() => navigate(`/trips/${trip.id}`)} sx={{ borderRadius: '30px', overflow: 'hidden', border: '1px solid #eee', cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', '&:hover': { transform: 'scale(1.01)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' } }}>
                    ...
                    <Box sx={{ bgcolor: '#dcdcdc', p: 2, px: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar src={trip.driver_profile?.avatar || undefined} sx={{ width: 40, height: 40, border: '2px solid white', bgcolor: '#c62828' }}>
                          {(!trip.driver_profile?.avatar && trip.driver_username) ? trip.driver_username[0].toUpperCase() : null}
                        </Avatar>
                        <Typography variant="body2" sx={{ fontWeight: 500 }}>{trip.driver_username || 'Nieznany'}</Typography>
                      </Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
